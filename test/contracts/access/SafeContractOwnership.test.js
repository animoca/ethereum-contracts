...
const config = {
  immutable: {name: 'SafeContractOwnershipMock', ctorArguments: ['initialOwner', 'forwarderRegistry'], testMsgData: true},
  diamond: {
    facets: [
      ...
      {
        name: 'SafeContractOwnershipFacetMock',
...
runBehaviorTests('SafeContractOwnership', config, function (deployFn) {
  let deployer, other;
  ...
  describe('functions', function () {
    ...
    describe('transferOwnership(address)', function () {
      ...
      context('when successful', function () {
        context('with the zero address as new owner', function () {
          beforeEach(async function () {
            await this.contract.transferOwnership(other.address);
            this.receipt = await this.contract.transferOwnership(ethers.ZeroAddress);
          });
          it('unsets the owner', async function () {
            expect(await this.contract.owner()).to.equal(ethers.ZeroAddress);
          });
          it('unsets the pending owner', async function () {
            expect(await this.contract.pendingOwner()).to.equal(ethers.ZeroAddress);
          });
          it('emits an OwnershipTransferred event', async function () {
        ...
        context('with a non-zero address as new owner', function () {
          ...
          it('updates the pending owner', async function () {
            expect(await this.contract.pendingOwner()).to.equal(other.address);
          });

          it('emits an OwnershipTransferPending event', async function () {
            await expect(this.receipt).to.emit(this.contract, 'OwnershipTransferPending').withArgs(other.address);
          });
        });
        ...
        context('with the current owner address as new owner', function () {
          ...
          it('does not update the pending owner', async function () {
            expect(await this.contract.pendingOwner()).to.equal(ethers.ZeroAddress);
          });
          ...
          it('does not emit an OwnershipTransferPending event', async function () {
            await expect(this.receipt).not.to.emit(this.contract, 'OwnershipTransferPending');
          });
        });
      });
    });
    ...
    describe('acceptOwnership()', function () {
      ...
      it('reverts if the caller is not the pending contract owner', async function () {
        await expect(this.contract.acceptOwnership())
          .to.be.revertedWithCustomError(this.contract, 'NotPendingContractOwner')
          .withArgs(deployer.address);
      });

      context('when successful', function () {
        beforeEach(async function () {
          this.receipt = await this.contract.connect(other).acceptOwnership();
        });
    ...
    describe('SafeContractOwnershipStorage.enforceIsContractOwner(address)', function () {
    ...
    describe('SafeContractOwnershipStorage.enforceIsTargetContractOwner(address,address)', function () {
