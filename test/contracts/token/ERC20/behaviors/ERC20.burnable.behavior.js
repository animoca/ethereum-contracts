...
function behavesLikeERC20Burnable(implementation) {
  ...
  describe('like a burnable ERC20', function () {
    ...
    describe('burnFrom(address,uint256)', function () {
      ...
      const shouldBurnTokens = function (fromIndex, value, senderIndex, withEIP717 = false) {
        context('when burning tokens', function () {
          beforeEach(async function () {
      ...
      const shouldBurnTokensBySender = function (value) {
        context('when burning started by the owner', function () {
          shouldBurnTokens(AccountIndex.owner, value, AccountIndex.owner);
        });

        context('when burning started by an approved sender', function () {
          shouldBurnTokens(AccountIndex.owner, value, AccountIndex.spender);
        });

        context('when burning started by a sender with max approval', function () {
          shouldBurnTokens(AccountIndex.owner, value, AccountIndex.maxSpender, features.EIP717);
        });
      };
    ...
    describe('batchBurnFrom(address[],uint256[])', function () {
      ...
      context('Pre-conditions', function () {
        ...
        it('reverts when one of the owners is the zero address', async function () {
          ...
          await expect(batchBurnFrom(this.contract.connect(spender), [owner.address, ZeroAddress], [One, One]), revertMessages.BurnExceedsAllowance);
        });
      ...
      const shouldBurnTokens = function (ownerIndexes, values, senderIndex, withEIP717 = false) {
        beforeEach(async function () {
          this.fromAllowances = {};
          this.fromBalances = {};
      ...
      const shouldBurnTokensBySender = function (ownerIndexes, values) {
        context('when burn started by the owner', function () {
          shouldBurnTokens(ownerIndexes, values, AccountIndex.owner);
        });

        context('when burn started by an approved sender', function () {
          shouldBurnTokens(ownerIndexes, values, AccountIndex.spender);
        });

        context('when burn started by a sender with max approval', function () {
          shouldBurnTokens(ownerIndexes, values, AccountIndex.maxSpender, features.EIP717);
        });
      };
      ...
      context('when burning a zero total value', function () {
        shouldBurnTokensBySender([AccountIndex.owner], [Zero]);
      });

      context('when burning zero values', function () {
        shouldBurnTokensBySender([AccountIndex.owner, AccountIndex.owner, AccountIndex.owner], [Zero, One, Zero]);
      });

      context('when burning the full allowance', function () {
        shouldBurnTokensBySender([AccountIndex.owner], [initialAllowance]);
        shouldBurnTokensBySender([AccountIndex.owner, AccountIndex.owner], [initialAllowance.sub(One), One]);
      });
    });

    if (features.ERC165 && interfaces.ERC20Burnable) {
      shouldSupportInterfaces(['IERC20Burnable']);
    }
  });
}
