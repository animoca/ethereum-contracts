...
describe('TokenMetadataResolverPerTokenERC1155', function () {
  ...
  const fixture = async function () {
    ...
    this.token = await deployContract('ERC1155Full', '', '', this.resolver.address, constants.AddressZero, await getForwarderRegistryAddress());
  ...
  describe('setTokenURI(address,uint256,string)', function () {
    ...
    it('the token contract emits a URI event', async function () {
      await expect(this.resolver.setTokenURI(this.token.address, 1, 'uri')).to.emit(this.token, 'URI').withArgs('uri', 1);
    });
  });
  ...
  describe('batchSetTokenURI(address,uint256[],string[])', function () {
    ...
    it('the token contract emits URI events', async function () {
      await expect(this.resolver.batchSetTokenURI(this.token.address, [1, 2], ['uri1', 'uri2']))
