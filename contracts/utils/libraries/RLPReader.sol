 ...
 * @dev ported to solidity 0.8 from https://github.com/hamdiallam/Solidity-RLP/blob/ba24e1213f720b1e6ab7b44848c38b40222b049f/contracts/RLPReader.sol
 */
library RLPReader {
    ...
    /**
     * Returns whether iteration has more elements.
     * @param it The iterator.
    ...
    /**
     * Creates an RLP item.
     * @param item the RLP encoded bytes.
     * @return the RLP item.
     */
    ...
    /**
     * Creates an iterator. Reverts if item is not a list.
     * @dev reverts if `item` is not a list
     * @param item The RLP encoded list.
    ...
    /**
     * A cheaper version of keccak256(toRlpBytes(item)) that avoids copying memory.
    ...
    /**
     * A cheaper version of keccak256(toBytes(item)) that avoids copying memory.
    ...
    function toBoolean(RLPItem memory item) internal pure returns (bool) {
        require(item.len == 1, "RLP: INVALID_BOOLEAN_LENGTH");
        uint256 result;
        uint256 memPtr = item.memPtr;
        assembly {
    ...
    function toAddress(RLPItem memory item) internal pure returns (address) {
        ...
        require(item.len == 21, "RLP: INVALID_ADDRESS_LENGTH");

        return address(uint160(toUint(item)));
    }

    function toUint(RLPItem memory item) internal pure returns (uint256) {
        require(item.len > 0 && item.len <= 33, "RLP: INVALID_UINT_LENGTH");
    ...
    function toUintStrict(RLPItem memory item) internal pure returns (uint256) {
        // one byte prefix
        require(item.len == 33, "RLP: INVALID_UINT_STRICT_LENGTH");
    ...
    function toBytes(RLPItem memory item) internal pure returns (bytes memory) {
        require(item.len != 0, "RLP: INVALID_BYTES_LENGTH");
